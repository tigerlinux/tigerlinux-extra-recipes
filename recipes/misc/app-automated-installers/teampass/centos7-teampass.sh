...
# TEAMPASS Server Installation Script
# Rel 1.1
...
OSFlavor='unknown'
lgfile="/var/log/teampass-server-automated-installer.log"
credfile="/root/teampass-server-credentials.txt"
...
export tpdbpass=`openssl rand -hex 10`
...
then
	echo "Not enough hardware for a TEAMPASS Server. Aborting!" &>>$lgfile
	echo "End Date/Time: `date`" &>>$lgfile
	exit 0
fi
...
CREATE DATABASE IF NOT EXISTS teampass default character set utf8;
GRANT ALL ON teampass.* TO 'teampass'@'%' IDENTIFIED BY '$tpdbpass';
GRANT ALL ON teampass.* TO 'teampass'@'127.0.0.1' IDENTIFIED BY '$tpdbpass';
GRANT ALL ON teampass.* TO 'teampass'@'localhost' IDENTIFIED BY '$tpdbpass';
FLUSH PRIVILEGES;
EOF
...
echo "TEAMPASS Database: teampass" >> $credfile
echo "TEAMPASS DB User: teampass" >> $credfile
echo "TEAMPASS DB User Password: $tpdbpass" >> $credfile
...
wget https://github.com/nilsteampassnet/TeamPass/archive/2.1.27.10.zip -O /root/tp.zip &>>$lgfile

unzip /root/tp.zip -d /usr/share/nginx/
mv /usr/share/nginx/TeamPass* /usr/share/nginx/html
chown -R root.root /usr/share/nginx/html
find /usr/share/nginx/html -name "*" -type f -exec chmod 644 "{}" ";"
find /usr/share/nginx/html -name "*" -type d -exec chmod 755 "{}" ";"
chown -R nginx.nginx /usr/share/nginx/html/includes/config
chown -R nginx.nginx /usr/share/nginx/html/includes/avatars
chown -R nginx.nginx /usr/share/nginx/html/includes/libraries/csrfp/libs
chown -R nginx.nginx /usr/share/nginx/html/includes/libraries/csrfp/log
chown -R nginx.nginx /usr/share/nginx/html/includes/libraries/csrfp/js
chown -R nginx.nginx /usr/share/nginx/html/backups
chown -R nginx.nginx /usr/share/nginx/html/files
chown -R nginx.nginx /usr/share/nginx/html/install
chown -R nginx.nginx /usr/share/nginx/html/upload
chown nginx.nginx /usr/share/nginx/html/includes

# Needed for Teampass
crudini --set /etc/php.ini PHP max_execution_time 60
...
systemctl enable php-fpm nginx crond

finalcheck=`curl --write-out %{http_code} --silent --output /dev/null http://127.0.0.1/|grep -c 302`

if [ $finalcheck == "1" ]
then
	echo "Ready. Your TEAMPASS Server is ready. See your database credentials at $credfile" &>>$lgfile
	echo "You will need the database credentials in order to complete the web installation:" &>>$lgfile
	echo "- Database host: 127.0.0.1, port 3306 (mysql type)" &>>$lgfile
	echo "- Database name: teampass" &>>$lgfile
	echo "- Database user: teampass" &>>$lgfile
	echo "- Database password: $tpdbpass" &>>$lgfile
	echo "" &>>$lgfile
	cat $credfile &>>$lgfile
	echo "End Date/Time: `date`" &>>$lgfile
else
	echo "TEAMPASS Server install failed" &>>$lgfile
	echo "End Date/Time: `date`" &>>$lgfile
fi
